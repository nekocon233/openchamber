...
- FRPC needs no per-tunnel CA configuration. OpenChamber collects the host runtime's trust roots (in Docker, the container's trust store; `SSL_CERT_FILE` and `NODE_EXTRA_CA_CERTS` are honored) into a private temporary PEM and injects it into FRPC, which still verifies the FRPS certificate over `transport.tls` with the server address as `serverName`. OpenChamber never learns a CA from an unauthenticated FRPS and never enables insecure certificate verification. If FRPS uses a private or enterprise CA, install that CA into the OS/container trust store of the machine running OpenChamber instead of configuring it per tunnel. For compatibility, CA bundles at the legacy host paths `~/.config/frp/ca.crt` and `~/.frp/ca.crt` (if present) are also merged into the collected trust roots; there is still no per-tunnel CA option in the UI, CLI, or environment.
