...
También se requiere un archivo de certificado de CA. FRPS debe presentar un certificado cuyo SAN coincida con `--frps-address`; OpenChamber verifica esa identidad y nunca desactiva la verificación del certificado.
...
El inicio directo usa los ajustes comunes del servidor FRPC y el token. Elige exactamente un endpoint: `OPENCHAMBER_TUNNEL_REMOTE_PORT` junto con `OPENCHAMBER_TUNNEL_PUBLIC_URL` para TCP, o `OPENCHAMBER_TUNNEL_CUSTOM_DOMAIN` y `OPENCHAMBER_TUNNEL_HOSTNAME` juntos para el host virtual HTTP.
