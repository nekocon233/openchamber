...
CA 証明書ファイルも必要です。FRPS は SAN が `--frps-address` と一致する証明書を提示する必要があり、OpenChamber はその ID を検証して証明書検証を無効にしません。
...
直接起動では、共通の FRPC サーバー設定とトークンを使います。エンドポイントは 1 つだけ選びます。TCP では `OPENCHAMBER_TUNNEL_REMOTE_PORT` と `OPENCHAMBER_TUNNEL_PUBLIC_URL`、HTTP 仮想ホストでは `OPENCHAMBER_TUNNEL_CUSTOM_DOMAIN` と `OPENCHAMBER_TUNNEL_HOSTNAME` の両方を使います。
