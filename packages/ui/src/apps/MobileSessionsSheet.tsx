...
const SortableProjectRow: React.FC<{
  ...
  onMoveWorktree: (path: string, toIndex: number, orderedPaths: string[]) => void;
}> = ({ project, totalSessions, expanded, onToggleExpanded, onMoveWorktree }) => {
  const { t } = useI18n();
  ...
  const handleWorktreeDragEnd = (event: DragEndEvent) => {
    ...
    onMoveWorktree(paths[fromIndex], toIndex, paths);
  };
...
export const MobileSessionsSheet: React.FC<MobileSessionsSheetProps> = ({ open, onOpenChange, variant = 'drawer', footer }) => {
  ...
  const sessions = React.useMemo(() => {
    ...
    // Archived sessions never show on mobile (no archived view here): a live
    // overlay can briefly retain one while the global list catches up.
                    ...
                    {projectsMeta.map((project) => {
                      ...
                      return (
                        <SortableProjectRow
                          ...
                          onMoveWorktree={(path, toIndex, orderedPaths) => (
                            moveWorktree(project.id, path, toIndex, orderedPaths)
                          )}
                        />
                      );
                    })}
