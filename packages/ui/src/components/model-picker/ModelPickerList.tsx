...
export type ModelPickerEntry = {
  ...
  | { kind?: 'model'; model: ProviderModel }
  | { kind: 'auto'; model: Pick<ProviderModel, 'id' | 'name'> }
);
...
const getModelDisplayName = (model: Pick<ProviderModel, 'id' | 'name'>) => {
...
export const ModelPickerList: React.FC<ModelPickerListProps> = ({
  ...
  const renderRow = (entry: ModelPickerEntry, keyPrefix: string, showProviderLogo: boolean, rowIndex: number, dragHandleProps?: SortableFavoriteHandleProps | null) => {
    const metadata = entry.kind === 'auto'
      ? undefined
      : mergeModelMetadataWithLiveModel(entry.providerID, entry.model, modelsMetadata.get(`${entry.providerID}/${entry.modelID}`));
