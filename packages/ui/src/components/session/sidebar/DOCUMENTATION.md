...
- `recent/` owns Pinned, Recent and managed Chats activity projections.
...
- `sessionSidebarRowModel.ts` owns the ordered, mode-neutral projection for
  Pinned, Chats, Recent, projects, groups, folders, sessions, status notices, empty
  states, and reveal controls. `projects/SessionProjectScroller.tsx` renders these
  rows on desktop Web, Electron and VS Code. Normal and committed-search modes use the same
...
Existing downstream layouts retain their animated running indicators during
migration. `SessionRunningIndicator` follows the same appearance preference.
...
Web and desktop show optional Pinned, managed Chats and optional Recent before
projects. Their visibility preferences are independent and device-local. All
three use the same row model and virtualizer, including search and reveal
...
Project structure, order, pins, folders and assignments follow the revisioned
`sidebarState` runtime API on web, desktop and mobile. Views submit mutations
through their owning stores. Selection, collapse state and display preferences
remain device-local. VS Code keeps its workspace-local fallback because its
runtime declares authoritative sidebar state unsupported.
