...
describe('useSessionProjectViewState', () => {
  ...
  test('keeps stable state/actions across unrelated parent updates', async () => {
    ...
    const Harness = ({ selection }: { selection: string }) => {
      ...
      const viewState = useSessionProjectViewState({ projects });
    ...
    try {
      await act(async () => root.render(React.createElement(Harness, { selection: 'initial' })));
      ...
      await act(async () => root.render(React.createElement(Harness, { selection: 'updated' })));
      expect(capture.renderCount).toBe(initialRenderCount + 1);
  ...
  test('preserves malformed group storage until explicit user mutation', async () => {
    ...
    const Harness = () => {
      capture.renderCount += 1;
      const value = useSessionProjectViewState({ projects: [{ id: 'project-a' }] });
