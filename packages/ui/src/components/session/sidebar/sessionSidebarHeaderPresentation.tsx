...
export const SessionSidebarActivityHeader: React.FC<{
  activityKey: 'chats' | 'active-now' | 'pinned';
  collapsed: boolean;
  ...
  const pinned = activityKey === 'pinned';
  return <div className="relative group/chats -ml-2.5 -mr-2">
    ...
    >
      <span className="inline-flex h-3.5 w-3.5 items-center justify-center">
        <Icon name={chats ? 'chat-4' : pinned ? 'pushpin' : 'history'} className={cn('h-3.5 w-3.5 text-muted-foreground/80', !forceExpanded && 'group-hover:hidden')} />
      ...
      <span className="typography-ui-label font-semibold lowercase text-foreground">
        {t(chats ? 'sessions.sidebar.activity.chatsTitle' : pinned ? 'sessions.sidebar.activity.pinnedTitle' : 'sessions.sidebar.activity.recentTitle')}
      </span>
    </button>
    {chats ? <button
      type="button"
