...
export type SessionSidebarRow =
  | (RowBase & { kind: 'activity-header'; activityKey: 'chats' | 'active-now' | 'pinned'; collapsed: boolean; forceExpanded: boolean })
  ...
  | (RowBase & { kind: 'session'; node: SessionNode; depth: number; projectId: string | null; groupDirectory: string | null; ownerKey: string | null; selectionScopeKey: string | null; archived: boolean; renderContext: 'project' | 'recent' | 'pinned'; secondaryMeta: SessionSidebarActivityItem['secondaryMeta'] })
...
export type SessionSidebarRowModelArgs = {
  ...
  pinnedItems?: readonly SessionSidebarActivityItem[];
...
const expandedKey = (renderContext: 'project' | 'recent' | 'pinned', archived: boolean, id: string): string => (
...
export const buildSessionSidebarRowModel = (args: SessionSidebarRowModelArgs): SessionSidebarRowModel => {
  ...
  const appendActivityHeader = (activityKey: 'chats' | 'active-now' | 'pinned'): boolean => {
  ...
  const appendActivitySection = (section: SessionSidebarActivitySection): void => {
    if (section.items.length === 0 || appendActivityHeader(section.key)) return;
    ...
    for (const item of visibleItems) {
      ...
      appendSessions({
        ...
        ownerKey: getSessionFolderOwnerKey(item.projectId, item.groupDirectory),
        selectionScopeKey: getSessionFolderOwnerKey(item.projectId, item.groupDirectory),
        archived: false, renderContext: section.key === 'pinned' ? 'pinned' : 'recent',
  ...
  if (args.pinnedItems) appendActivitySection({ key: 'pinned', items: args.pinnedItems });
  if (args.chatGroup) {
  ...
  if (args.showRecentSection) {
    for (const section of args.recentSections) appendActivitySection(section);
  }

  let projectSections = args.singleProjectMode
