...
const node = (id: string, children: SessionNode[] = []): SessionNode => ({
  session: { id } as SessionNode['session'],
  children,
  worktree: null,
});
...
describe('parent expansion persistence', () => {
  test('hydrates the complete v3 set and preserves unknown/context-isolated entries when toggling', async () => {
    ...
    try {
      ...
      await act(async () => mounted.capture.value!.toggleParent('project:active:parent-a', node('parent-a')));
  ...
  test('leaves durable data unchanged on write failure and rereads it on remount', async () => {
    ...
    await act(async () => first.capture.value!.toggleParent('project:active:parent-b', node('parent-b')));
  ...
  test('collapsing a parent clears descendants only in the same render context', async () => {
    const initial = [
      'project:active:parent',
      'project:active:child',
      'project:active:grandchild',
      'recent:active:child',
      'project:archived:child',
    ];
    const storage = createStorage(JSON.stringify(initial));
    const mounted = await mountHook(storage);
    const tree = node('parent', [node('child', [node('grandchild')])]);
    try {
      await act(async () => mounted.capture.value!.toggleParent('project:active:parent', tree));
