...
describe('chatDraftPersistence', () => {
  beforeEach(() => {
    ...
    storage.removeItem('oc.chatDraft.local.v2:runtime-a:session%3Asession-1');
  });
  ...
  test('migrates the shipped downstream local draft without resurrecting it after an edit', () => {
    const identity = createChatDraftIdentity('runtime-a', '/repo', 'session-1')!;
    const legacyKey = 'oc.chatDraft.local.v2:runtime-a:session%3Asession-1';
    storage.setItem(legacyKey, JSON.stringify({
      version: 2,
      text: 'legacy draft',
      confirmedMentions: ['legacy.ts'],
      generation: 3,
      updatedAt: 42,
    }));

    expect(readChatDraft(identity)).toEqual({
      text: 'legacy draft',
      confirmedMentions: new Set(['legacy.ts']),
    });
    expect(storage.getItem(legacyKey)).not.toBeNull();

    writeChatDraft(identity, 'edited draft', []);
    expect(storage.getItem(legacyKey)).toBeNull();
    expect(readChatDraft(identity).text).toBe('edited draft');
  });
