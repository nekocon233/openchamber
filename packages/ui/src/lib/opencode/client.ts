...
import { assertProviderCircuitClosed, captureProviderTrackerLane, recordProviderError, recordProviderSuccess } from "./provider-tracker"
...
class OpencodeService {
  ...
  async sendMessage(params: {
    ...
    try {
      ...
      recordProviderError(providerTrackerLane, params.providerID, error instanceof OpencodeApiError ? error.status : undefined)
      throw error
    }
   ...
   */
  async getActiveSessionStatuses(
    directory?: string | null,
    options?: { signal?: AbortSignal; timeoutMs?: number },
  ): Promise<Record<string, SessionStatus> | null> {
    const timeout = createTimeoutSignal(options?.timeoutMs ?? 4_000, options?.signal)
    try {
      const client = isSpaceDirectory(directory) && directory ? this.getScopedSdkClient(directory) : this.client
      const active = activeSessionSnapshotSchema.parse(await call("session.active", () => client.session.active({ signal: timeout.signal })))
