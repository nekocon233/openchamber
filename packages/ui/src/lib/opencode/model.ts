...
export type UserMessage = MessageBase & {
  role: "user"
  time: { created: number }
  /** Native CLIs retain the user's selected alias and effort on each prompt. */
  agent?: string
...
export type AssistantMessage = MessageBase & {
  ...
  agent: string
  /** Native CLI transcript ancestry and its folded compaction summary. */
  parentID?: string
  summary?: boolean
  providerID: string
  modelID: string
...
export type TextPart = PartBase & {
  ...
  metadata?: Metadata
  /** Native CLI context can arrive attached to a user message. */
  synthetic?: boolean
}
...
/** A compaction boundary from a native CLI transcript. */
export type CompactionPart = PartBase & { type: "compaction"; auto: boolean }

export type Part = TextPart | ReasoningPart | FilePart | AgentPart | ToolPart | CompactionPart
