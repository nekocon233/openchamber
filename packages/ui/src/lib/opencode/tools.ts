 ...
 */
// Native Claude/Codex sessions retain their CLI tool names at the boundary.
const NATIVE_TOOL_ALIASES: ReadonlyMap<string, string> = new Map([
  ['bash', OPENCODE_TOOLS.shell],
  ['task', OPENCODE_TOOLS.subagent],
  ['apply_patch', OPENCODE_TOOLS.patch],
  ['multiedit', OPENCODE_TOOLS.edit],
]);
...
export function normalizeToolName(toolName: ToolName): string {
  ...
  const name = segments[segments.length - 1] ?? withoutIndex
  return NATIVE_TOOL_ALIASES.get(name) ?? name
}
