...
describe('openSessionFromRoute', () => {
  ...
  test('uses an explicit cold-start directory without replacing it from another snapshot', async () => {
    useGlobalSessionsStore.setState({
      activeSessions: [buildSession(SESSION_ID, OTHER_DIR)],
      archivedSessions: [],
      hasLoaded: true,
      status: 'ready',
    });

    await openSessionFromRoute(SESSION_ID, PROJECT_DIR);
