...
import { normalizeFrpcHostname, normalizeFrpcPublicUrl } from '@/components/sections/openchamber/frpcTunnelSettings';
...
/** frpc port: `null` clears; an integer in 1–65535 keeps it. */
const parseFrpcPort: SettingsParser<number | null> = (value) => {
  if (value === null) return null;
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 1 || value > 65535) return undefined;
  return value;
};

/** frpc public URL: `null` clears; '' becomes null; anything that is not a bare https origin is dropped. */
const parseFrpcPublicUrl: SettingsParser<string | null> = (value) => {
  if (value === null) return null;
  if (typeof value !== 'string') return undefined;
  if (!value.trim()) return null;
  const normalized = normalizeFrpcPublicUrl(value);
  return normalized || undefined;
};

/** frpc hostname/custom domain: `null` clears; '' becomes null; an unparseable host is dropped. */
const parseFrpcHostname: SettingsParser<string | null> = (value) => {
  if (value === null) return null;
  if (typeof value !== 'string') return undefined;
  if (!value.trim()) return null;
  const normalized = normalizeFrpcHostname(value);
  return normalized || undefined;
};
...
export const SETTINGS_REGISTRY = {
  ...
  // ── frpc tunnel provider (instance) ──
  frpcProxyType: field({ scope: 'instance', parse: parseOneOf(['tcp', 'http']) }),
  frpcServerAddress: field({ scope: 'instance', parse: parseNullableTrimmedPath }),
  frpcServerPort: field({ scope: 'instance', parse: parseFrpcPort }),
  frpcRemotePort: field({ scope: 'instance', parse: parseFrpcPort }),
  frpcPublicUrl: field({ scope: 'instance', parse: parseFrpcPublicUrl }),
  ...
  conciseTranscript: field({ scope: 'profile', parse: parseBoolean, ui: uiStore('conciseTranscript', (v) => useUIStore.getState().setConciseTranscript(v)) }),
  transcriptFileChangesOnly: field({ scope: 'profile', parse: parseBoolean, ui: uiStore('transcriptFileChangesOnly', (v) => useUIStore.getState().setTranscriptFileChangesOnly(v)) }),
