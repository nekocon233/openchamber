...
`useUIStore.ts` owns only the `followUpBehavior` delivery preference and its startup projection. Successful server settings synchronization remains authoritative, and the preference selects official OpenCode `queue` or `steer` delivery; UI stores do not own staged follow-up content, a queue protocol, an outbox, or claims. The shipped `queueModeEnabled` and `immediate` settings remain read-only preference migrations, not queue compatibility. Legacy custom queue storage is intentionally left untouched and inert so removing the client queue cannot lose data or replay a message twice.
