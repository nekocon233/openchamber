...
import { recordSessionRemoval } from '@/sync/session-event-freshness';
...
export const useGlobalSessionsStore = create<GlobalSessionsState>((set, get) => ({
  ...
  loadSessions: async (fallbackActive) => {
    ...
    const loadPromise = (async () => {
      ...
      try {
        ...
        const [allSessions, nativeSessions] = await Promise.all([listGlobalSessionPages(listSessionPage, {
          pageSize: PAGE_SIZE,
  ...
  refreshSessionsForDirectories: async (directories, fallbackActive) => {
    ...
    const [fetched, nativeSessions] = await Promise.all([
      fetchDirectoryPages(directorySet),
