...
| `input-store.ts` | Pending input injections, attached files, synthetic parts | App UI state |
...
| `messageQueueStore.ts` | Follow-up queue snapshots, optimistic operations, claims, and local fallback | Per runtime and session; active OpenChamber host is authoritative when supported |
...
`messageQueueStore.ts` watches only mounted sessions and owns the cross-device follow-up queue. For supported runtimes it loads a host-authoritative `{scopeToken, revision, items}` snapshot, persists each semantic operation under an independent stable mutation key before dispatch, projects operations optimistically, serializes one mutation per client lane, and replays the same intent over a typed 409 snapshot. Independent records prevent concurrent tabs from overwriting each other's offline outbox. Failed loads retain the previous authoritative snapshot and optimistic outbox. Opaque `openchamber:follow-up-queue.changed` events trigger only the matching scope load; terminal hints can explicitly reset a stale persisted revision. Reconnect and transport readiness resume loads and outbox drain. Every async completion is gated by runtime key and generation, and confirmed deletion drops the queue lane and all mutation records. VS Code and mixed-version hosts without the capability use a runtime-scoped device-local queue instead of proxying content to OpenCode; fallback auto-send requires a cross-context browser lock.

Each staged item captures attachments and send configuration plus one stable OpenCode-compatible `messageId`. Sending requires a host claim: auto claims only `queued` items, manual claims either status, and the host clock decides whether a competing claim has expired. Confirmed sends complete and remove the item; failures release the claim, restoring `staged` for automatic sends and the original status for manual sends. Claim completion is bound to the runtime generation that acquired it. Attachment persistence strips browser `File`; loaded items rebuild an empty placeholder while `dataUrl`, MIME type, filename, source metadata, optional paths, configuration, and order remain intact.
...
5. Failed optimistic archive/delete actions restore only the affected session at its prior position. Never restore after a newer authoritative removal event, and never restore a captured whole list over concurrent events or newly created sessions.
