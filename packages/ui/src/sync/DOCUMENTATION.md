...
Cross-directory running indicators combine child-store status with the global status event store. Directory status snapshots carry both a global revision baseline and per-session child-status references so delayed HTTP responses cannot overwrite newer events or optimistic send/rollback transitions. Optimistic sends receive a short grace period against an overtaking idle snapshot; failed sends still clear immediately. Authoritative reconnect snapshots may clear missing sessions and also record that the directory has completed a status snapshot, so an assistant left incomplete by an interrupted process cannot masquerade as live activity after restart. Monotonic periodic watchdog checks publish only active `busy`/`retry` entries and never infer idle from absence. Explicit global `idle` overrides stale child activity while retained in the bounded recent-resolution history, and authoritative snapshots also clear child state. Compaction removes the oldest terminal history first and never evicts genuinely active entries to satisfy the 2,000-entry history limit. Bootstrap and watchdog status requests are scoped, validated, time-bounded, and cancelled or ignored across runtime changes. Runtime reset clears active entries, retained idle resolutions, snapshot markers, and per-session revisions, then advances the revision floor against late responses.
...
Follow-up queue state is owned by OpenChamber, not by OpenCode. Busy normal prompts whose follow-up behavior is `queue` are added to `stores/messageQueueStore.ts` with captured content, attachments, provider, model, agent, variant, and a stable message ID. Web and desktop use the revisioned host queue API with outbox replay, cross-client claims, and revision hints; VS Code reports an explicit unsupported queue API and uses the runtime-scoped local fallback. Entries may be removed, edited back into the composer, marked staged/queued, reordered, claimed, released, completed, or sent manually. Idle auto-drain uses the captured configuration and ordinary `promptAsync`; failed sends release the item instead of dropping it. `steer` remains an immediate OpenCode delivery option and keeps the optimistic transcript reconciliation path.

Unsubmitted composer content remains device-local UI state. Runtime switches re-key queue lanes and never replay one runtime's entries into another. Session deletion drops the local lane and tombstones persisted queue data. Separately, the event pipeline translates supported `session.next.*` prompt, assistant, text/reasoning, and tool events into legacy message/part/status events; initial and tail message loads also merge projected V2 messages missing from the legacy transcript while legacy history remains the older-page authority.
