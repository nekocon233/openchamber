...
Initial loads use smaller pages on constrained VS Code/mobile surfaces. Legacy and V2 older pages are fetched through the same loader with independent cursors, then merged with optimistic records before publication. If one source fails, records from the successful source are still materialized while the load remains retryable and neither source cursor advances. When a V2 page begins with an assistant whose user message is on the next older page, the merged history fills only the missing `parentID` after that user arrives.
