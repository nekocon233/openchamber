...
Follow-up delivery is owned by the official OpenCode API. `session-ui-store.ts` routes submissions through `opencodeClient.sendMessage()` and may pass OpenCode's `delivery: "steer" | "queue"` option; `optimisticSend()` only projects the submitted user message and reconciles SDK and event results. The sync layer has no local follow-up queue authority: it does not store queue snapshots or outbox mutations, claim or auto-dispatch items, subscribe to `openchamber:follow-up-queue.changed`, or resume queue work from transport-ready hooks. Reconnect behavior remains the normal OpenCode event and message reconciliation path.

Unsubmitted composer content remains device-local UI state. Once submitted, ordering and pending delivery are OpenCode-owned and become visible through official message and status responses and events; runtime switches and session deletion therefore clear only OpenChamber UI state and do not mutate a separate local delivery queue.
