...
import { heldNativeForms, heldNativeStatuses, readNativeForms, readNativeStatuses } from './native-directory-snapshots';
import {
...
async function initializeDirectory(input: DirectoryBootstrapInput): Promise<BootstrapResult> {
  ...
  const critical = Promise.allSettled([
    read(async () => {
      ...
      const session_status = await readDirectoryStatusSnapshot(store, async () => {
        const [openCode, native] = await Promise.all([
          opencodeClient.getActiveSessionStatuses(directory),
          readNativeStatuses(directory),
        ])
        if (openCode === null) throw new Error("session.active failed")
    ...
    read(async () => {
      const form = await readDirectoryFormSnapshot(store, async () => {
        const [openCode, native] = await Promise.all([
          opencodeClient.listPendingForms({ directories: [directory], includeGlobal: false }),
          readNativeForms(directory),
        ])
        return [...openCode, ...(native ?? heldNativeForms(store.getState().form))]
      })
      commit({ form })
    }),
    read(async () => {
