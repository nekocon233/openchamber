...
type ConfirmationReadResult =
  | { outcome: "success"; records: Array<{ info: Message; parts: Part[] }> }
  | { outcome: "failure"; error: Error }
let sessionMessagesResult: ConfirmationReadResult = { outcome: "success", records: [] }
...
mock.module("@/lib/opencode/client", () => ({
  ...
  opencodeClient: {
    ...
    getSessionMessages: mock(async (
      ...
      if (sessionMessagesResult.outcome === "failure") throw sessionMessagesResult.error
      return { items: sessionMessageRecords.get(sessionId) ?? sessionMessagesResult.records, cursor: {} }
    }),
...
mock.module("@/lib/messages/contextParts", () => ({
  CONTEXT_METADATA_KEY: 'openchamberContext',
...
mock.module("@/lib/runtime-fetch", () => ({
  runtimeFetch: async (path: string, init?: { body?: string }) => {
    ...
    if (path.endsWith('/archive') && sessionUpdatePromise) {
      const session = await sessionUpdatePromise
      return Response.json({ archived: [{ id: session.id, archivedAt: session.time.archived }], failedIds: [] })
    }
...
describe("session removal navigation runtime scope", () => {
  ...
  test("does not send a delete through the replacement runtime while prerequisite work is pending", async () => {
    ...
    setActionRefs(createChildStores([["/test/project", oldStore]]),
      () => "/test/project",
    )
    ...
    setActionRefs(createChildStores([["/test/project", newStore]]),
      () => "/test/project",
    )
  ...
  test("preserves the session and concurrent updates until deletion is confirmed", async () => {
    ...
    setActionRefs(createChildStores([["/test/project", store]]),
      () => "/test/project",
    )
    const deletion = deleteSession("session-a")
  ...
  test("rejects an archived response after stores rebind to another runtime", async () => {
    ...
    while (!openchamberRouteRequests.some((call) => call.path.endsWith("/archive"))) await Promise.resolve()
  ...
  test("does not clean either runtime after a stale delete response", async () => {
    ...
    const { deleteSession, setActionRefs } = await import("./session-actions")
    setActionRefs(createChildStores([]), () => "/test/project")
...
describe("optimisticSend unconfirmed outcome", () => {
  const UNREACHABLE_MESSAGES_RESULT: ConfirmationReadResult = { outcome: "failure", error: Object.assign(new Error("fetch failed"), { status: 502 }) }
  ...
  const startUnconfirmedSend = async (targetStore: TestStoreApi<DirectoryStore>, childStores: ReturnType<typeof createChildStores>, options?: {
    sessionId?: string
  }) => {
  ...
  beforeEach(() => {
    replyCalls.length = 0
    sessionMessagesResult = { outcome: "success", records: [] }
  })
