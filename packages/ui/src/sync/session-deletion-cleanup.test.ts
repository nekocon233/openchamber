...
describe('cleanupPersistedSessionState', () => {
  ...
  test('clears persisted state only for the deleted composite session', () => {
    ...
    useMessageQueueStore.setState({
      queuedMessages: {
        [getMessageQueueKey(deleted)]: [{ id: 'q-delete', messageId: null, content: 'delete', createdAt: 1, status: 'queued' as const }],
        [getMessageQueueKey(retained)]: [{ id: 'q-retain', messageId: null, content: 'retain', createdAt: 1, status: 'queued' as const }],
      },
    });
