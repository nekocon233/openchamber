...
const createRecord = (
  sessionID: string,
  id = "msg_1",
  roleOrCreated: "user" | "assistant" | number = "user",
  parentID?: string,
) => ({
  info: {
    id,
    sessionID,
    role: typeof roleOrCreated === "number" ? "user" : roleOrCreated,
    ...(parentID ? { parentID } : {}),
    time: { created: typeof roleOrCreated === "number" ? roleOrCreated : 1 },
  } as Message,
...
describe("SessionMessageLoader", () => {
  ...
  test("queues initial history behind an unresolved reconnect tail refresh", async () => {
    ...
    expect(requests).toEqual([{ limit: 30, cursor: undefined }])
  ...
  test("does not retarget delayed retries after the loader runtime changes", async () => {
    ...
    let oldCalls = 0
    let newCalls = 0
    const newSdk = { getSessionMessages: async () => { newCalls += 1; return response([]) } }
    const oldSdk = { getSessionMessages: async () => {
      oldCalls += 1
