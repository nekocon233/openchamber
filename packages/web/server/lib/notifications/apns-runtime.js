...
export const createApnsRuntime = (deps) => {
  ...
  const addOrUpdateApnsToken = async (authValue, deviceToken, userAgent, platform, environment) => {
    // Older internal callers supplied a raw UI session token. Convert it to an
    // opaque identity; delivery still requires authoritative auth validation.
    const auth = normalizeNotificationAuth(authValue)
      || createUiSessionNotificationAuth(authValue);
