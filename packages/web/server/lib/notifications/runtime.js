...
export const createNotificationTriggerRuntime = (deps) => {
  ...
  const hasActiveSessionGoal = async (sessionId, directory) => {
    if (!sessionId || (!nativeSessions?.isNativeSessionId(sessionId) && !readSessionMetadata)) return false;
    try {
      const metadata = nativeSessions?.isNativeSessionId(sessionId)
        ? (await readSessionRecord(sessionId, directory))?.metadata
