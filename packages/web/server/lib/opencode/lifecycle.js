...
export const createOpenCodeLifecycleRuntime = (deps) => {
  ...
  const probeOpenCodeTcpPort = (port, timeoutMs) => new Promise((resolve) => {
    if (!port || port <= 0) {
      resolve({
        ...
        failure: {
          class: 'error',
          detail: 'Managed OpenCode TCP port is unavailable',
        },
      });
      return;
    }
    ...
    const timer = setTimeout(() => finish({
      ...
      failure: {
        class: 'timeout',
        detail: `TCP health gate timed out after ${timeoutMs}ms`,
      },
    }), timeoutMs);
    socket.once('connect', () => finish({ healthy: true, failure: null }));
  ...
  const probeOpenCodeHealthDetailed = async () => {
    ...
    // its full timeout.
    const tcpResult = await probeOpenCodeTcpPort(state.openCodePort, HEALTH_CHECK_TCP_PROBE_TIMEOUT_MS);
