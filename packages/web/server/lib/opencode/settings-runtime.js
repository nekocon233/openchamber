...
export const createSettingsRuntime = (deps) => {
  ...
  const cleanupSettingsTempFilesOnce = async () => {
  ...
  const readSettingsFromDiskMigrated = async ({ surface = null } = {}) => {
    await cleanupSettingsTempFilesOnce();
    const settings = await migrateSettingsFromDiskSnapshot(await readSettingsFromDisk());
    ...
    return normalizeSettingsSurface(surface) ? readSettingsFromDisk({ surface }) : settings;
  };

  const readSettingsFromDiskMigratedStrict = async ({ surface = null } = {}) => {
    await cleanupSettingsTempFilesOnce();
    const settings = await migrateSettingsFromDiskSnapshot(await readSettingsFromDiskStrict());
    // Migrations run on the base view; a surface asks for its own resolution
    // of the per-surface keys on top of the migrated files.
    return normalizeSettingsSurface(surface) ? readSettingsFromDisk({ surface }) : settings;
  };
