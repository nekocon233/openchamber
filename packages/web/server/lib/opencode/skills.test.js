...
describe('skills', () => {
  ...
  it('rolls back the directory rename when frontmatter write fails', async () => {
    ...
    let writeFileSpy;
    let observedMovedDirectory = false;
    ...
    try {
      ...
      writeFileSpy = vi.spyOn(fs, 'writeFileSync').mockImplementation((filePath, ...args) => {
        if (filePath === renamedPath) {
          observedMovedDirectory = fs.existsSync(renamedDir) && !fs.existsSync(skillDir);
          throw Object.assign(new Error('injected SKILL.md write failure'), { code: 'EIO' });
        }
        return writeFileSync(filePath, ...args);
      });

      expect(() => renameSkill('rollback-skill', 'rollback-skill-renamed', projectRoot))
        .toThrow('Failed to write agent markdown file');

      expect(observedMovedDirectory).toBe(true);
