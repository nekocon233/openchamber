...
function readNativeMessage({ info, parts }) {
  const blocks = [];
  const authored = [];
...
function collectTurns(messages) {
  ...
  for (const message of messages) {
    ...
    if (message.role === 'user') {
      ...
      attached = [];
      if (!text) {
        if (active) parents.add(message.id);
        continue;
      }
    ...
    attached = [];
    if (!active || message.role !== 'assistant' || message.summary) continue;
 ...
 */
export async function loadAssistContext({ readPage, signal, native = false }) {
  let messages = [];
  let cursor;
  ...
  for (let pageNumber = 0; pageNumber < MAX_PAGES; pageNumber++) {
    ...
    signal.throwIfAborted();
    const records = native ? page?.records : page?.data;
    if (!Array.isArray(records)) throw new Error('Session message page is unavailable');
    const older = [];
    for (const record of records) {
      const id = native ? record?.info?.id : record?.id;
      if (!id || ids.has(id)) continue;
      ids.add(id);
      older.push(native ? readNativeMessage(record) : readMessage(record));
    }
    if (!native) older.reverse();
    messages = older.concat(messages);
    const next = native ? page.cursor : page.cursor?.next ?? null;
