...
async function fixture(generate = async () => output()) {
  ...
  const server = createServer(async (request, response) => {
    ...
    state.requests.push({ method: request.method, directory: decodeURIComponent(request.headers['x-opencode-directory'] ?? ''), limit: url.searchParams.get('limit'), auth: request.headers['x-test-auth'] });
    response.setHeader('content-type', 'application/json');
    if (request.method === 'PATCH') {
      const chunks = [];
      for await (const chunk of request) chunks.push(chunk);
      state.patches.push(JSON.parse(Buffer.concat(chunks).toString()));
      response.end(JSON.stringify({ data: { ...state.session, location: { directory: state.session.directory } } }));
    } else if (url.pathname.endsWith('/message')) {
      const records = [...state.messages].reverse().slice(0, Number(url.searchParams.get('limit'))).map(({ info, parts }) => ({
        ...info,
        type: info.role,
        model: { providerID: info.providerID, id: info.modelID },
        text: info.role === 'user' ? parts.filter(part => part.type === 'text' && !part.synthetic).map(part => part.text).join('\n') : undefined,
        content: parts,
      }));
      response.end(JSON.stringify({ data: records, cursor: {} }));
    } else {
      state.gets++;
  ...
  const runtime = createSessionAssistRuntime({
    ...
    isSessionArchived: async () => Boolean(state.session.time.archived),
    persistSessionAssist: async (_sessionId, _directory, assist) => {
      state.patches.push({ metadata: { ...state.session.metadata, openchamber: { ...state.session.metadata.openchamber, assist } } });
    },
...
describe('session assist runtime', () => {
  ...
  it('uses bounded authenticated SDK reads and preserves metadata with an empty suggestion', async () => {
    ...
    expect(state.requests.filter((r) => r.limit).map((r) => r.limit)).toEqual(['50', '8']);
  ...
  it('does not write a stale result when the tail moves during generation', async () => {
    ...
    status('idle');
    await vi.waitFor(() => expect(state.requests.some((r) => r.limit === '8')).toBe(true));
    await pause();
    expect(state.patches).toHaveLength(0);
  });
