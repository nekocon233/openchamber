...
An explicit request or Small Model setting may select `claude-native/<model>`
or `codex-native/<model>`. `configureNativeSmallModels` receives those utility
runtimes from the native session backend. They own login, generation and
structured output and remain available when OpenCode is unreachable. Each
runtime contributes its own available provider independently.

A native session has no automatic small-model fallback. It requires an
explicit selection, so a background action never spends another provider's
subscription silently. A selected native model that is missing or signed out
fails without falling back. `describeSmallModel` reports the CLI's login,
transport, structured-output support and model limits. The generation and
preview cap the output reserve to the same limit, and input clamping includes
system instructions. See `../native-agents/DOCUMENTATION.md`, "Utility text
generation", for the CLI process and tool restrictions.
