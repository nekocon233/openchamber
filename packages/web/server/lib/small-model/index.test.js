...
describe('generateSmallModelText', () => {
  ...
  it('keeps the session provider unless the caller explicitly permits switching', async () => {
    state.models = [
      MODEL({ id: 'claude-sonnet-5', family: 'claude-sonnet' }),
      MODEL({ id: 'gemini-3.6-flash', providerID: 'google', family: 'gemini-flash' }),
    ];
    expect(await generateSmallModelText({
      prompt: 'hi', directory: '/proj', preferredProviderID: 'anthropic', preferredModelID: 'claude-sonnet-5',
    })).toMatchObject({ providerID: 'anthropic', modelID: 'claude-sonnet-5', source: 'session-model' });
  });

  it('requires an explicit small model for a native session', async () => {
    await expect(generateSmallModelText({
      prompt: 'hi', preferredProviderID: 'claude-native', preferredModelID: 'opus',
    })).rejects.toMatchObject({ statusCode: 404 });
    expect(await describeSmallModel({ preferredProviderID: 'claude-native', preferredModelID: 'opus' })).toBeNull();
...
describe('oversized input', () => {
  ...
  it('counts system instructions before generation and retains their space when truncating', async () => {
    const input = { prompt: 'x'.repeat(15_000), system: 's'.repeat(2_000), directory: '/proj' };
    await expect(generateSmallModelText({ ...input, onOverflow: 'error' })).rejects.toMatchObject({
      statusCode: 413, requiredChars: 17_000, availableChars: 16_000,
    });
    expect(lastGenerate()).toBeUndefined();
    expect(await generateSmallModelText(input)).toMatchObject({ inputTruncated: true });
    expect(lastGenerate().body.prompt).toHaveLength(16_002);
    expect(lastGenerate().body.prompt.startsWith(input.system + '\n\n')).toBe(true);
  });
...
describe('describeSmallModel', () => {
  it('caps the preview reserve to the same output limit used for generation', async () => {
    expect(await describeSmallModel({ outputReserveTokens: 20_000 })).toMatchObject({
      outputTokens: 4_000, inputCharBudget: 16_000,
    });
    await expect(generateSmallModelText({ prompt: 'x'.repeat(16_001), maxOutputTokens: 20_000, onOverflow: 'error' }))
      .rejects.toMatchObject({ availableChars: 16_000 });
  });
